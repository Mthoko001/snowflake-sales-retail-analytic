...
SELECT
    ...
    CASE
        WHEN TRY_TO_NUMBER(TRIM(quantity)) IS NULL THEN 0
        WHEN TRY_TO_NUMBER(TRIM(quantity)) = 0 THEN 0
        WHEN TRY_TO_NUMBER(TRIM(unit_price)) IS NULL THEN 0
        ELSE
            TRY_TO_NUMBER(TRIM(quantity))
            * TRY_TO_NUMBER(TRIM(unit_price))
    END AS line_total
